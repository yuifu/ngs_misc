...
function gtfToBed12(path_gtf, path_bed)
	...
	# d_start = Dict{AbstractString, IntSet}()
	# d_end = Dict{AbstractString, IntSet}()
	# d_codon = Dict{AbstractString, IntSet}()
...
function calcBlockSizes(set_start::IntSet, set_end::IntSet)
	join([string(collect(set_end)[i] - collect(set_start)[i] + 1) * "," for i in range(1, length(set_start))], "")
end

function calcBlockStarts(set_start::IntSet, chromStart)
	join([string(i - chromStart) * "," for i in set_start ], "")
end

function calcThickStart(set_codon::IntSet, chromEnd)
...
function calcThickEnd(set_codon::IntSet, chromEnd)
	if length(set_codon) == 0
		chromEnd
	else
		last(set_codon)
...
function calcLine(
					chr::AbstractString, set_start::IntSet, set_end::IntSet, 
...
path_gtf = ARGS[1]
path_bed = ARGS[2]
# path_gtf =  "/home/harukao/projects/eRNA/data/GENCODE/gencode.vM9.annotation.gtf"
# path_bed =  "/home/harukao/projects/eRNA/data/GENCODE/gencode.vM9.annotation.gtf.bed"
